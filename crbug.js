class CrbugSearcher extends Searcher {
  ...
  getSuggestions(response) {
    ...
    if (this.#isBugQuery()) {
      const { textContent: summary } = dom.querySelector(
        "#issueheader span.h3"
      );
      if (!summary) return [];

      const { textContent: owner } = dom.querySelector(
        "#issuemeta tr:nth-child(2) a.userlink"
      );

      return [
        {
          content: this.getBugURL_(),
          description: this.#getBugDescription(
            summary,
            owner ?? "--",
            this.query
          ),
    ...
    for (const row in results) {
      ...
      const { textContent: owner } =
        row.getElementsByClassName("col_7")[0] ?? {};
      if (!owner) return;

      const { textContent: summary } =
        row.getElementsByClassName("col_8")[1] ?? {};
      if (!summary) return;
